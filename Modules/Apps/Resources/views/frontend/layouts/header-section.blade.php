            ...
            <div class="container">
                <div class="topp">
                    ...
                    <ul class="header-top-right">
                        ...
                        <li class="menu-item-has-children">
                            <form method="post" action="{{ route('frontend.home.update_currency') }}">
                                @csrf
                                <select style="padding: 5px; border: none;background:transparent" onchange="this.form.submit()"
                                        name="currency_id">
                                    @php
                                        $current_currency = isset(session()->get('currency_data')['selected_currency']) ? optional(session()->get('currency_data')['selected_currency'])->id : defaultCurrency('id');
                                    @endphp
                                    @foreach ($supported_currencies as $key => $currency)
                                        <option value="{{ $key }}"
                                            {{ $current_currency == $key ? 'selected' : '' }}>
                                            {{ $currency }}
                                        </option>
                                    @endforeach
                                </select>
